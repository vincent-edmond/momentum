...
// ─── EtapeCard (composant interne, lié à la logique de déverrouillage) ────────
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] text-[#0A0A0F] pb-24">
      {/* Sticky header */}
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-[#E2E4EA] px-4 py-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
          <span className="text-[#0046FF] font-black text-lg tracking-tight flex-shrink-0">
            MOMENTUM
          </span>
          <div className="flex items-center gap-3 min-w-0">
            <span
              className={`text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${planColor.badge}`}
            >
              {chemin.icon} {chemin.titre}
            </span>
            <span className="text-xs text-[#9096A5] hidden sm:block truncate">
              {chemin.horizon}
            </span>
          </div>
          <span className="text-sm font-bold text-[#0046FF] flex-shrink-0">
            {progression.score}%
          </span>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 pt-8 space-y-10">
        ...
        <section>
          <h1 className="text-2xl sm:text-3xl font-black text-[#0A0A0F] mb-1">
            Bienvenue, {session.prenom}. 👋
          </h1>
          <p className="text-[#555B6E] text-sm mb-6">
            Ton chemin :{" "}
            <span className="text-[#0A0A0F] font-semibold">{chemin.titre}</span>{" "}
            <span className="text-[#9096A5]">·</span>{" "}
            {chemin.horizon}
          </p>
        ...
        <section>
          <div className="flex items-center gap-3 mb-4">
            <div className="w-7 h-7 rounded-full bg-[#0046FF] flex items-center justify-center text-white text-xs font-black">
              1
            </div>
            <h2 className="text-lg font-black text-[#0A0A0F]">
        ...
        {result.contenu.etapes.length > 0 && (
          <section>
            ...
            {/* DEV : les étapes 2 et 3 seront verrouillées en production jusqu'à validation de l'étape précédente */}
        ...
        {/* Bloc accélération : naturel, pas vendeur */}
        ...
        {result.contenu.lecture && (
          <section>
            ...
            {/* DEV : cette section sera verrouillée en production, accessible uniquement après avoir regardé la vidéo principale */}
            <LectureCard
